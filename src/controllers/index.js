        ...
        dataBarisBuku: {
            ...
            detail: async function(req, res) {
                try {
                    ...
                    return res
                        .status(200)
                        .render("pages/koleksi/data-baris-buku-detail", {
                            data: {
                                appName,
                                book,
                            },
    ...
    pelayanan: {
        bukuTamu: {
            ...
            detail: function(req, res) {
                ...
                if (validId.length === 0)
                    return res.status(403).redirect("/pelayanan/buku-tamu");
            ...
            post: async function(req, res) {
                try {
                    ...
                    const currentUTCOffsetInMilliseconds =
                        todayUTC.getTimezoneOffset() * 60 * 1000;
                    const utcPlus7OffsetInMilliseconds = 7 * 60 * 60 * 1000;
                    const todayInUTCPlus7 = new Date(
                        todayUTC.getTime() +
                        currentUTCOffsetInMilliseconds +
                        utcPlus7OffsetInMilliseconds
                    );
        ...
        sumbangBuku: {
            ...
            post: async function(req, res) {
                try {
                    const { judul, jumlah, penulis, kategori, tahun, deskripsi, link } =
                    req.body;
                    if (!judul ||
                        !jumlah ||
                        !penulis ||
        ...
        post: async function(req, res) {
            try {
                ...
                const token = jwt.sign({
                    ...
                    environment.jwt.secret, { expiresIn: "30d" }
                );
    ...
    logout: {
        post: async function(req, res) {
            try {
                ...
                return res
                    .status(200)
                    .json({ status: 200, message: "Logout berhasil" });
            } catch (error) {
                return res
                    .status(500)
                    .json({ status: 500, message: "Terjadi kesalahan pada server" });
    ...
    admin: {
        get: async function(req, res) {
            ...
            const startOfDay = new Date(
                todayInUTCPlus7.getFullYear(),
                todayInUTCPlus7.getMonth(),
                todayInUTCPlus7.getDate(),
                0,
                0,
                0,
                0
        ...
        buku: {
            ...
            add: {
                ...
                post: async function(req, res) {
                    try {
                        const { judul, deskripsi, kategori, penulis, tahun, link, stock } =
                        req.body;
                        if (!judul ||
                            !deskripsi ||
                            !kategori ||
